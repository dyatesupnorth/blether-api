...
class UserController extends BaseController
{
    ...
    public function update()
    {
        ...
        if ($validator->passes()) {
            ...
            if (!$user->save())
                $user = ApiResponse::errorInternal('An error occurred. Please, try again.');
    ...
    {
        $users = User::where('firstname', 'LIKE', '%' . Input::get('query') . '%')->get();
...
//        $friendships = User::find('user_friends')->get($user->_id);
        $friendships = Friend::where('user_id', $user->_id)->get();

        $friendships->each(function ($friendship) {
            $friendship->friend_info = User::find($friendship->friend_id);
